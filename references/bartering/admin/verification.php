...
<body>
  ...
  <main class="main-content pt-3">
    <div class="container main-title mb-5">
      ...
      <div>
        <nav class="nav">
          <a class="nav-link text-sm" href="verification.php">Pending Verification</a>
          <a class="nav-link text-dark text-sm" href="verification-history.php">Verification History</a>
        </nav>
      <div class="table-responsive border p-4 border shadow rounded">
        <table id="myTable" class="w-100 table table-striped">
          ...
          <tbody>
            <?php
              $query = "SELECT * FROM verifications INNER JOIN users ON verifications.user_id = users.user_id WHERE verifications.verification_status = 'pending' ORDER BY verification_id DESC";
