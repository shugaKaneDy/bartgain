...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
  ...
  $numDays = htmlspecialchars($_POST['numDays']);
  ...
  if($numDays < 5) {
    errorResponse("Please put a valid number of days");
  }

  if($itemInfo['item_user_id'] != $_SESSION['user_details']['user_id']) {
    errorResponse("Invalid item");
  }

  $cost = floor($numDays / 5) * 120 + ($numDays % 5) * 30;
  $cost *= 100;
  ...
  try {
    ...
    insertQuery(
      ...
      [
          ...
          ':paymentItemId' => $itemInfo['item_id'],
