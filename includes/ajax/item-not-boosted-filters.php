...
if (!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
    ...
    if($function == 'search') {
        ...
        $search = $_GET['search'] ?? '';  // Use null coalescing operator to avoid errors if search is not set
        ...
        $boostedItems = selectQuery(
            ...
            AND (items.item_title LIKE :search OR items.item_description LIKE :search)
            ...
            [
                ...
                ':itemBoosted' => "No",
                ':search' => '%' . $search . '%'  // Add wildcards to enable partial matching
            ]
        );
    } elseif($function == 'category') {
        $cat = $_GET['cat'] ?? '';
        ...
        $boostedItems = selectQuery(
            ...
            WHERE items.item_boosted = :itemBoosted
            AND items.item_status = 'available'
            AND (items.item_category LIKE :cat)
            ORDER BY items.item_flagged ASC, users.user_is_prem = 'Yes' DESC, distance ASC
            LIMIT $offset, $itemsPerPage",
            [
                ':lat' => $_SESSION["user_details"]["lat"],
                ':lng' => $_SESSION["user_details"]["lng"],
                ':itemBoosted' => "No",
                ':cat' => '%' . $cat . '%'  // Add wildcards to enable partial matching
            ]
        );
        
    } elseif($function == 'nearest') {
        ...
        $boostedItems = selectQuery(
            ...
            WHERE items.item_boosted = :itemBoosted
            AND items.item_status = 'available'
            ORDER BY items.item_flagged ASC, distance ASC, users.user_is_prem = 'Yes' DESC, category_priority DESC
            LIMIT $offset, $itemsPerPage",
            [
                ':lat' => $_SESSION["user_details"]["lat"],
                ':lng' => $_SESSION["user_details"]["lng"],
    ...
    if (count($boostedItems) > 0) {
                ...
                <div class="p-2 bg-white rounded border">
                    ...
                    data-bs-toggle="modal" data-bs-target="#itemModalView">
                        <div class="">
                            ...
                            <?php else: ?>
                                <video src="item-uploads/<?= $firstFile ?>" class="image-container rounded"></video>
                            <?php endif ?>
                        </div>
        ...
        // Display test items when no data is available
        for ($i = 0; $i < $itemsPerPage; $i++) {
            ?>
            <div class="col-6 col-md-3 col-xl-2 p-0 p-1 position-relative d-flex align-items-stretch">
                <div class="p-2 bg-white rounded border">
                    <div class="">
                        <img src="item-uploads/test-product.png" class="image-container rounded">
                    </div>
