...
<body class="bg-light">
  ...
  <main>
    <div class="container-xl">
      ...
      <?php if($itemPartner['meet_up_status'] == 'cancelled'): ?>
        <div class="row p-3 border-top mb-2 justify-content-center">
          <div class="col-12 col-md-8">
            <p class="m-0">Status:
                <span class="fw-bold text-danger"><?= $itemPartner['meet_up_status'] ?></span>
            </p>
            <div class="w-100 border border-danger border-2 p-2 mb-3 rounded shadow">
              ...
              <p class="mb-2 ps-3 pe-2 text-secondary"><?= $itemPartner['meet_up_cancel_reason'] ?></p>

            </div>
          </div>
        </div>
      <?php elseif($itemPartner['meet_up_status'] == 'completed'): ?>
        <div class="row p-3 border-top mb-2 justify-content-center">
          <div class="col-12 col-md-8">
            <p class="m-0">Status:
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form id="cancelMeetupForm">
          <input type="hidden" name="offer_id" value="<?= $itemPartner['offer_random_id'] ?>">
    ...
    $(document).ready(function() {
      ...
      function updateMeetupLocation() {
          ...
          $.ajax({
              ...
              data: {
                  partnerId: <?= json_encode($itemPartner['user_random_id']) ?>
              },
              dataType: "JSON",
      ...
      <?php if ($itemPartner['meet_up_status'] == 'cancelled'): ?>
      <?php elseif ($itemPartner['meet_up_status'] == 'completed'): ?>
      // Don't execute further if the meet-up is completed
      <?php else: ?>
          initializeMap();
