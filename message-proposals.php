  ...
  if(isset($_GET["offer_id"])) {
    if(!empty($_GET["offer_id"])) {
      ...
      updateQuery(
        ...
        [
          ":messageOfferId" => $itemPartner['offer_id'],
          ":messageUserId" => $itemPartner['item_user_id'],
        ]
      );
    } else {
    ...
    header("Location: message-proposals.php?offer_id=" . $pagePartner['offer_random_id']);
  }
    ...
    <div class="row p-0 m-0 h-100">
      <div class="col-12 col-md-3 chats border-end pt-2 d-flex flex-column d-md-block">
        ...
        <div class="row m-0 p-0">
          <div class="col-6 px-1">
            <a href="message-offers.php" class="btn btn-sm btn-outline-success rounded-pill w-100">
              Offers
            </a>
          </div>
          <div class="col-6 px-1">
            <a href="message-proposals.php" class="btn btn-sm btn-success rounded-pill w-100">
              Proposals
            </a>
          </div>
        </div>
      ...
      <div class="col-12 col-md-6 message border-end d-md-block h-100">
        <div class="row border-bottom first-top">
          <div class="col-8 p-2 d-flex align-items-center">
            <img src="profile-uploads/<?= isset($itemPartner['profile_picture']) ? $itemPartner['profile_picture'] : 'default.jpg' ?>" alt="" class="img-fluid rounded-pill" style="height: 40px; width: 40px">
            <div class="ps-2">
              <p class="fw-bold m-0"><?= isset($itemPartner['fullname']) ? $itemPartner['fullname'] : '' ?></p>
        ...
        <div class="row border-bottom p-2 second-top shadow-sm">
          <?php if(isset($itemPartner['item_random_id'])): ?>
            <div class="col-6 p-1">
              <button value="<?= $itemPartner['item_random_id'] ?>" class="forViewItemModal btn btn-outline-secondary w-100"
            ...
            <div class="col-6 p-1">
              <button value="<?= $itemPartner['offer_random_id'] ?>" class="forViewOfferModal btn btn-outline-success bg-green w-100"
        ...
        <div class="row bg-white p-2 border-top bottom-top">
          <?php if(!empty($itemPartner['item_status']) && $itemPartner['item_status'] == 'completed'):?>
          <p class="text-warning text-center m-0">This item has already been bartered.</p>
          <?php elseif(empty($itemPartner['offer_cancelled_reject_reason'])):?>
      ...
      <div class="col-12 col-md-3 plan d-md-block pt-2 pb-3">
        ...
        <div class="border rounded p-2 mb-3">
          ...
          <?php if(isset($itemPartner['offer_url_file'])): ?>
            <div class="row align-items-end">
              ...
              <div class="col-8">
                <p class="fw-bold m-0"><?= $itemPartner['offer_title'] ?></p>
                <button value="<?= $itemPartner['offer_random_id'] ?>" class="forViewOfferModal btn btn-sm btn-outline-success bg-green w-100"
                data-bs-toggle="modal" data-bs-target="#offerModalView"
                >
                View Details</button>
              </div>
        ...
        <div>
          <?php if(empty($itemPartner['offer_status'])): ?>
          <?php elseif($itemPartner['offer_status'] == 'accepted' && isset($itemPartner['offer_url_file'])): ?>
            <h5 class="text-center bg-success rounded py-2 text-white">Offer Accepted</h5>
            <div class="w-100 d-flex justify-content-center mb-3">
              <a href="meet-up-sender.php?offer_id=<?= $getOfferId ?>" class="link-success">View Meet Up <i class="bi bi-geo-alt-fill"></i></a>
            </div>
            <div class="w-100 d-flex justify-content-center">
              <img src="assets/offer-accepted.png" style="height: 180px">
            </div>
          <?php elseif($itemPartner['offer_status'] == 'rejected' && isset($itemPartner['offer_url_file'])): ?>
            <h5 class="text-center bg-danger rounded py-2 text-white">Offer Rejected</h5>
            <p class="m-0">Reject Reason: </p>
            <p class="m-0 ps-3 pe-2 text-secondary"><?= $itemPartner['offer_cancelled_reject_reason'] ?> </p>
          <?php elseif($itemPartner['item_status'] == 'completed' && isset($itemPartner['offer_url_file'])): ?>
            <h5 class="text-center bg-warning rounded py-2 text-white">This item has already been bartered.</h5>
          <?php elseif(isset($itemPartner['offer_url_file'])): ?>
            <form id="planForm" action="">
              <input name="planLng" id="planLng" type="hidden" value="<?= $itemPartner['offer_lng'] ?>">
              <input name="planLat" id="planLat" type="hidden" value="<?= $itemPartner['offer_lat'] ?>">
              <input name="itemOfferForPlan" id="itemOfferForPlan" type="hidden" value="<?= $getOfferId ?>">
              <div class="form-floating mb-3">
                <input name="locationMeetUp" id="locationMeetUp" type="text" class="form-control my-input" id="locationMeetUp" placeholder="location" value="<?= $itemPartner['offer_meet_up_place'] ?>" required>
                <input type="hidden" name="locationMeetUpReal" id="locationMeetUpReal" value="<?= $itemPartner['offer_meet_up_place'] ?>">
                <label for="locationMeetUp">Landmark</label>
              </div>
              <div id="suggestions" class="bg-white"></div>
              <div id="map" class="w-100 mb-3"></div>
              <div class="form-floating mb-1">
                <input itemid="dateMeetup" name="dateMeetup" type="datetime-local" class="form-control my-input" id="dateMeetup" placeholder="Birth date" required value="<?= $itemPartner['offer_date_time_meet'] ?>">
    ...
    $(document).ready(function() {
      ...
      function renderMessage(numIndicator) {
        ...
        $.ajax({
          ...
          url: "includes/ajax/messages-proposal.inc.php"
        }).done(res => {
          $('.third-top').html(res);
          if(numIndicator == 1) {
      ...
      function renderChats() {
        ...
        $.ajax({
          ...
          method: 'POST',
          data: {offerId: <?= $getOfferId ?>},
          url: "includes/ajax/chats-proposals.inc.php"
        }).done(res => {
          $('.chatTable').html(res);
        })
      ...
      <?php if($itemPartner['item_status'] == 'completed'): ?>
      <?php elseif($itemPartner['offer_status'] == 'rejected'): ?>
      <?php elseif(empty($itemPartner['offer_lng'])): ?>
        yourCurrentLocation();
      <?php elseif($itemPartner['offer_status'] == 'pending'): ?>
        meetupLocationDirection(
                                  <?= json_encode($itemPartner['offer_lng']) ?>,
                                  <?= json_encode($itemPartner['offer_lat']) ?>,
                                  <?= json_encode($itemPartner['offer_meet_up_place']) ?>
                                )
      <?php else: ?>
      <?php endif; ?>
